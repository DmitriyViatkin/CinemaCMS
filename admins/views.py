...
def new_contacts(request):
    seo_instance = Block_SEO.objects.filter(seo_url='contact_paige').first()
    if not seo_instance:
        seo_instance = Block_SEO.objects.create(title_seo ="contact_paige", seo_url = 'contact_paige', seo_description ="SEO описание для страницы контактов")
